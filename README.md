...
3. 桌面端默认使用 WASD 移动、自动索敌普通攻击、空格释放主动诀技；移动端提供基础触控入口。
...
src/
  ...
  weapons/              唯一的 WeaponRuntime 武器定义与运行时契约
...
项目已经具备可运行的核心玩法垂直切片：20 波昼夜流程、四把武器、六类敌人、五个 Boss、八种属性及 23 种反应均有实际运行路径；四把武器统一由 `WeaponRuntime` 提供契约，局内 HUD 按局势、成长、自身战斗和玩家本局 Build 分层展示，怪物负面状态会显示目标级图标、层数、剩余时间和轻量属性反馈。Build 只由玩家本局选卡自然形成，不提供 Build 管理或商店。剩余工作主要集中在 Android 正式发布配置、触控体验、音频、数值平衡、真机和多分辨率验收，以及发行材料准备；其中音频、发布和外围系统不属于当前核心收口目标。详见 [`docs/PLAN.md`](docs/PLAN.md) 与 [`docs/TASKS.md`](docs/TASKS.md)。

核心自动化验证可使用 `godot --headless --path . --editor --quit` 检查工程加载，再按 [`docs/PLAN.md`](docs/PLAN.md) 中的测试场景运行逻辑、流程、内容、武器和构筑回归。视觉 Smoke Test 在有渲染设备时保存截图；headless 环境只报告场景启动通过，不将空渲染纹理当作画面验收。
